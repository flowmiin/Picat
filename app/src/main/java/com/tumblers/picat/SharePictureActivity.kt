    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        UserApiClient.instance.me { user, error ->
            if (error != null) {
                ...
                joinFriendList.add(FriendData(myKakaoId, ImageData(0, myPicture?.toUri()!!), myNickname!!))
                ...
                mSocket?.emit("participate", FriendData(myKakaoId, ImageData(0, myPicture?.toUri()!!), myNickname!!))
    ...
    var onParticipate = Emitter.Listener { args ->
        CoroutineScope(Dispatchers.Main).launch {
            ...
            for (i in 0..friend_list.length() - 1) {
                val profile = JSONObject(friend_list[i].toString()).getString("url").toUri()
